...
unsafe fn hpke_encap_bytes(
    ...
    let recipient_pub =
        unsafe { read_slice(recipient_pub_ptr, recipient_pub_len, "recipient_public_key") }?;
    let info = unsafe { read_slice(info_ptr, info_len, "info") }?;
    let aad = unsafe { read_slice(aad_ptr, aad_len, "aad") }?;
    let plaintext = unsafe { read_slice(plaintext_ptr, plaintext_len, "plaintext") }?;
...
unsafe fn hpke_decap_bytes(
    ...
    let info = unsafe { read_slice(info_ptr, info_len, "info") }?;
    let aad = unsafe { read_slice(aad_ptr, aad_len, "aad") }?;
    let enc = unsafe { read_slice(enc_ptr, enc_len, "enc") }?;
    let ciphertext = unsafe { read_slice(ciphertext_ptr, ciphertext_len, "ciphertext") }?;
