...
interface StrongBoxExports {
  ...
  strong_box_hpke_encap(
    recipientPubPtr: number,
    recipientPubLen: number,
    infoPtr: number,
    infoLen: number,
    aadPtr: number,
    aadLen: number,
  ...
  strong_box_hpke_decap(
    recipientPrivPtr: number,
    recipientPrivLen: number,
    infoPtr: number,
    infoLen: number,
    aadPtr: number,
    aadLen: number,
...
ctx.addEventListener('message', (event: MessageEvent<WorkerRequest>) => {
  ...
  readyPromise
    ...
    .catch((error) => postError(request.id, error))
})
...
async function handleRequest(request: WorkerRequest) {
  ...
  switch (request.op) {
    case 'decrypt': {
      ...
      const cacheKey = await computeStrongBoxCacheKey(key, context, payload)
    ...
    case 'hpke_encap': {
      const resultBytes = executeHpke('encap', {
        recipientKey: new Uint8Array(request.recipientPublicKey),
        info: new Uint8Array(request.info),
        aad: new Uint8Array(request.aad),
    ...
    case 'hpke_decap': {
      const resultBytes = executeHpke('decap', {
        recipientKey: new Uint8Array(request.recipientPrivateKey),
        info: new Uint8Array(request.info),
        aad: new Uint8Array(request.aad),
        enc: new Uint8Array(request.enc),
        payload: new Uint8Array(request.payload),
      })
      const buffer = resultBytes.buffer as ArrayBuffer
      ctx.postMessage(
...
async function initWasm() {
  ...
  if (!response.ok) {
    throw new Error(`Failed to load StrongBox WASM from ${wasmUrl}: ${response.status}`)
  }
  ...
  wasm = instance.instance.exports as unknown as StrongBoxExports
